from metrics import e_recall, nmi, f1, mAP, mAP_c, mAP_1000, mAP_lim, mAP_r
    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, **kwargs):
        ...
        with torch.no_grad():
            ...
            for idx,inp in enumerate(final_iter):
                # print (len(inp.values()))
                input_img, target, _, _ = inp.values()
                target_labels.extend(target.numpy().tolist())
                out, _, _ , _= model(input_img.to(device), False)
                if isinstance(out, tuple): out, rnnout, aux_f = out
