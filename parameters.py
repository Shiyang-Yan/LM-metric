...
def basic_training_parameters(parser):
    ...
    parser.add_argument('--dataset',              default='sop',   type=str,   help='Dataset to use. Currently supported: cub200, cars196, online_products.')
    ...
    parser.add_argument('--augmentation',         default='big',   type=str,   help='Type of preprocessing/augmentation to use on the data.  \
    ...
    parser.add_argument('--decay',             default=0.0001,   type=float,        help='Weight decay placed on network weights.')
    ...
    parser.add_argument('--bs',                default=180,     type=int,          help='Mini-Batchsize to use.')
    ...
    parser.add_argument('--loss',            default='smoothap',      type=str,   help='Training criteria: For supported methods, please check criteria/__init__.py')
    ...
    parser.add_argument('--not_pretrained',   action='store_true',  default=False,               help='Flag. If set, no ImageNet pretraining is used to initialize the network.')
    parser.add_argument('--arch',             default='resnet50_normalize_frozen', type=str,   help='Underlying network architecture. Frozen denotes that \
    ...
    parser.add_argument('--evaluate_on_gpu',  default =True,   help='Flag. If set, all metrics, when possible, are computed on the GPU (requires Faiss-GPU).')
    parser.add_argument('--evaluation_metrics', nargs='+', default=['e_recall@1', 'e_recall@2', 'e_recall@4', 'nmi', 'f1', 'mAP_r', 'mAP_1000', 'mAP_lim', 'mAP_c'], type=str, help='Metrics to evaluate performance by.')
    ...
    parser.add_argument('--source_path',  default='../CUB/data/',   type=str, help='Path to training data.')
    parser.add_argument('--save_path',    default=os.getcwd()+'/sop_smoothap', type=str, help='Where to save everything.')
...
def loss_specific_parameters(parser):
    ...
    parser.add_argument('--loss_margin_beta_constant', default=True,        help='Flag. If set, beta-values are left untrained.')
...
def batch_creation_parameters(parser):
    parser.add_argument('--data_sampler',              default='hierarchical', type=str, help='How the batch is created. Available options: See datasampler/__init__.py.')
    parser.add_argument('--samples_per_class',         default=4,              type=int, help='Number of samples in one class drawn before choosing the next class. Set to >1 for tuple-based loss.')
