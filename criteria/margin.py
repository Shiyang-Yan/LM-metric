    ...
    def forward(self, batch, rnn_outputs, labels, phi, **kwargs):
        ...
        if len(sampled_triplets):
            ...
            for triplet in sampled_triplets:
                ...
                pos_dist = phi[0][triplet[0]]*((train_triplet['Anchor']-train_triplet['Positive']).pow(2).sum()+1e-8).pow(1/2)
                neg_dist = phi[1][triplet[0]]*((train_triplet['Anchor']-train_triplet['Negative']).pow(2).sum()+1e-8).pow(1/2)
...
###########################################################################################################
        sampled_triplets = self.batchminer(rnn_outputs, labels)

        if len(sampled_triplets):
             d_ap, d_an = [], []
             for triplet in sampled_triplets:
                 train_triplet = {'Anchor': rnn_outputs[triplet[0], :], 'Positive': rnn_outputs[triplet[1], :],
                                  'Negative': rnn_outputs[triplet[2]]}
                 pos_dist = phi[0][triplet[0]]*((train_triplet['Anchor'] - train_triplet['Positive']).pow(2).sum() + 1e-8).pow(1 / 2)
                 neg_dist = phi[1][triplet[0]]*((train_triplet['Anchor'] - train_triplet['Negative']).pow(2).sum() + 1e-8).pow(1 / 2)
             ...
             if self.beta_constant:
                 ...
                 beta = torch.stack([self.beta[labels[triplet[0]]] for triplet in sampled_triplets]).to(torch.float).to(
                     d_ap.device)
             ...
             if pair_count == 0.:
                 loss_rnn = torch.sum(pos_loss + neg_loss)
             else:
                 loss_rnn = torch.sum(pos_loss + neg_loss) / pair_count

             if self.nu:
                 beta_regularization_loss = torch.sum(beta)
                 loss_rnn += self.nu * beta_regularisation_loss.to(torch.float).to(d_ap.device)
        else:
                 loss_rnn = torch.tensor(0.).to(torch.float).to(rnn_outputs.device)
