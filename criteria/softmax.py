...
class Criterion(torch.nn.Module):
    ...
    def forward(self, batch, labels, phi, **kwargs):
        ...
        #logprob = F.log_softmax(class_mapped_batch, -1)
        logprob = phi.unsqueeze(-1) * F.log_softmax(class_mapped_batch, -1)

        loss = (F.nll_loss(logprob, labels.to(torch.long).to(self.par.device)))
