...
class SmoothRankAP(nn.Module):
    ...
    def quick_forward(self, phi, scores, target):
        batch_size = target.size(0)
        device = scores.device
        scores = scores
        ...
        sim_sg = sim_diff_sigmoid * mask * phi
    ...
    def forward(self, batch, rnn_outputs, labels, phi, force_general=False, verbose=False, **kwargs):
        scores = torch.mm(batch, batch.t())
        target = lib.create_label_matrix(labels).cuda()
        scores_rnn = torch.mm(rnn_outputs, rnn_outputs.t()) 
        assert len(scores.shape) == 2

       # if (scores.size(0) == scores.size(1)) and not force_general:
        ap = self.quick_forward(phi[0].unsqueeze(1), scores, target)
        ap_rnn = self.quick_forward(phi[0].unsqueeze(1), scores_rnn, target)

       # else:
       #     ap = self.general_forward(phi[0].unsqueeze(1), scores, target, verbose=verbose)

        if self.return_type == 'AP':
            return ap+ap_rnn
        elif self.return_type == 'mAP':
            return ap.mean()+ap_rnn.mean()
        elif self.return_type == '1-AP':
            return 1 - ap + 1 - ap_rnn
        elif self.return_type == '1-mAP':
            return 1 - ap.mean() + 1 - ap_rnn.mean()
...
class Criterion(SmoothRankAP):

    def __init__(self, opt, tau=0.01, **kwargs):
