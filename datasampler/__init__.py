...
import datasampler.hierarchical_sampler

def select(sampler, opt, image_dict, mode='train', image_list=None, **kwargs):
    ...
    elif 'hierarchical' in sampler:
        sampler_lib = hierarchical_sampler
    else:
        raise Exception('Minibatch sampler <{}> not available!'.format(sampler))
    sampler = sampler_lib.Sampler(opt, image_dict,mode = mode, image_list=image_list)
