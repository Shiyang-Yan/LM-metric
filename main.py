...
if __name__ == '__main__':
    ...
    for epoch in range(opt.n_epochs):
        ...
        if warm > 0:
                  unfreeze_model_param = list(model.model.mapping.parameters()) + list(model.model.smachine.parameters()) \
                  + list(model.model.decision_fcsp.parameters()) + list(model.model.decision_fcsn.parameters()) \
                  + list(model.model.transform.parameters()) \
                  + list(model.model.gflow.parameters()) \
                  ...
                  for param in list(list(model.parameters()) + list(criterion.parameters())):
                          param.requires_grad = True
        ...
        for i, out in enumerate(data_iterator):
            ...
            input, class_labels, _, _= out.values()
            meta_train = True
            ### Compute Embedding
            input      = input.to(opt.device)
            model_args = {'x':input.to(opt.device), 'meta_train': meta_train}
            # Needed for MixManifold settings.
            if 'mix' in opt.arch: model_args['labels'] = class_labels
            embeds, phi, prob, gan_loss = model(**model_args)

            if isinstance(embeds, tuple): embedding, rnn_output, (avg_features, features) = embeds
            ### Compute Loss
            loss_args['batch'] = embedding
            loss_args['rnn_outputs']          = rnn_output
            loss_args['labels']         = class_labels
            loss_args['f_embed']        = model.model.mapping
            ...
            with torch.no_grad():
                distmat = inner_product(embedding.detach(), embedding.detach())
                reward = torch.tensor(AP_evaluation(distmat.cpu().numpy(),
                class_labels.cpu().numpy(), class_labels.cpu().numpy()))
            reward = (reward)
            rl_loss = (-prob[0] * torch.tensor(reward).cuda()).mean()# + (-prob[1] * torch.tensor(reward).cuda()).mean()

            loss_all = loss + gan_loss #+ rl_loss
            ###
            optimizer.zero_grad()
            loss_all.backward()
